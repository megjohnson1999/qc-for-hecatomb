...
rule generate_per_sample_coverage:
    ...
    shell:
        ...
        # First, get contig lengths
        seqkit fx2tab -nl {input.contigs} | awk '{{print $1 "\\t" 0 "\\t" $2}}' > temp_contigs.bed
        
        # Generate coverage in windows
        bedtools makewindows -b temp_contigs.bed -w {params.window_size} | \
        bedtools coverage -a - -b {input.bam} -mean > {output.coverage} 2> {log}
