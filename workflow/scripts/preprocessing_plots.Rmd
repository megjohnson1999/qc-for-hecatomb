...
## PrimerB contamination removal (Claude's code)
```{r, include=FALSE}
# Read in the data
...
# Approach 1: Boxplots with adjusted y-axis scale
# First, determine the appropriate y-axis limit based on your data
...
# Approach 2: Combined violin plot with individual points
primer_b_long <- primer_b_clean %>%
  select(sample, percent_with_primerB, percent_with_primerB_rc, percent_total_primerB) %>%
  pivot_longer(
    cols = -sample,
    names_to = "metric",
...
primer_b_long$metric <- factor(
  primer_b_long$metric,
  levels = c("percent_with_primerB", "percent_with_primerB_rc", "percent_total_primerB"),
  labels = c("PrimerB", "PrimerB_rc", "Total PrimerB")
)

# Create violin plot with points
ggplot(primer_b_long, aes(x = metric, y = percentage, fill = metric)) +
  ...
  labs(
    title = "PrimerB Contamination Distribution",
    x = NULL,
    y = "Percent of reads (%)"
  )

# Approach 3: Ranked bar chart (for seeing individual samples)
# Focus on total PrimerB contamination
top_samples <- primer_b_clean %>%
  arrange(desc(percent_total_primerB)) %>%
  mutate(rank = row_number(),
         sample_short = ifelse(nchar(sample) > 15, 
                              paste0(substr(sample, 1, 12), "..."), 
                              sample)) %>%
  # If too many samples, limit to top 30
  filter(rank <= min(30, n()))

# Create horizontal bar chart
ggplot(top_samples, aes(x = reorder(sample_short, percent_total_primerB), 
                       y = percent_total_primerB)) +
  geom_col(fill = "steelblue") +
  geom_text(aes(label = sprintf("%.3f%%", percent_total_primerB)), 
            hjust = -0.1, size = 3) +
  coord_flip() +
  theme_bw() +
  theme(
  ...
  labs(
    title = "Top Samples by Total PrimerB Contamination",
    x = NULL,
    y = "Percent of reads (%)"
  )

# Approach 4: Log scale visualization (especially good for very small values)
# Create log scale boxplots
p1_log <- ggplot(primer_b_clean, aes(x="", y=percent_with_primerB)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(width=0.2, alpha=0.6) +
  theme_bw() +
  scale_y_log10(
    labels = scales::percent_format(scale = 1),
    breaks = scales::trans_breaks("log10", function(x) 10^x)(c(0.01, max_percent))
  ) +
  annotation_logticks(sides = "l") +
  ylab("Percent of reads (log scale)") +
  xlab("") +
  labs(title="PrimerB contamination (log scale)")

p2_log <- ggplot(primer_b_clean, aes(x="", y=percent_with_primerB_rc)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(width=0.2, alpha=0.6) +
  theme_bw() +
  scale_y_log10(
  ...
  ylab("Percent of reads (log scale)") +
  xlab("") +
  labs(title="PrimerB_rc contamination (log scale)")

# Display log scale plots side by side
...
```{r, include=FALSE}
# Read in the data
vector_path <- file.path(snakemake@input[["vector"]])
vector <- read_tsv(vector_path)
```

```{r, echo=FALSE, warning=FALSE}
# Create the plot for vector contamination with data validation
vector_clean <- vector %>%
  mutate(percent_with_vector = pmax(0, pmin(100, percent_with_vector)))

ggplot(vector_clean, aes(x="", y=percent_with_vector))+
  geom_boxplot(outlier.shape=NA)+
  geom_jitter(width=0.2) +
  theme_bw() +
  ylab("Percent of reads with vector") +
  xlab("") +
  scale_y_continuous(limits = c(0, 100)) +
  labs(title="Vector contamination")

# Plot top vector types if available
if ("top1_vector" %in% colnames(vector)) {
  # Gather the top vector data
  top_vectors <- vector %>%
    select(sample, contains("top"), -contains("percent")) %>%
    pivot_longer(
      cols = contains("top"),
      names_to = c("rank", "type"),
      names_pattern = "top(\\d+)_(vector|count)",
      values_to = "value"
    ) %>%
    pivot_wider(
      names_from = type,
    ...
    filter(!is.na(vector) & !is.na(count) & count > 0)
  
  if (nrow(top_vectors) > 0) {
    # Plot the top vectors
    ggplot(top_vectors, aes(x = reorder(vector, -count), y = count)) +
      geom_bar(stat = "identity") +
      theme_bw() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      labs(title = "Most common vector contaminants", x = "Vector", y = "Count")
  }
}
```
