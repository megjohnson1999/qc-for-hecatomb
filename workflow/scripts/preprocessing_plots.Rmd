...
ggplot(raw_stats, aes(x="", y=num_seqs/1000000)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(width=0.2, aes(color=sample)) +
  theme_bw() +
...
ggplot(raw_stats, aes(x="", y=avg_len)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(width=0.2, aes(color=sample)) +
  theme_bw() +
  theme(legend.position="none") +
...
# Create a plot showing reads per sample
read_counts <- raw_stats %>%
  # Identify R1 files to avoid double-counting paired reads
  filter(grepl("_R1\\.fastq\\.gz", file)) %>%
  # Sort by number of reads descending
  arrange(desc(num_seqs))

ggplot(read_counts, aes(x=reorder(sample, -num_seqs), y=num_seqs/1000000)) +
  geom_bar(stat="identity", fill="steelblue") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
  xlab("Sample") +
  ylab("Number of reads (millions)") +
  labs(title="Raw reads per sample")
```
...
ggplot(vector_clean, aes(x = "", y = percent_with_vector, color = short_vector_label)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  theme_bw() +
  ...
  theme(legend.text = element_text(size = 10), # Adjust legend text size
        legend.key.size = unit(0.5, "cm")) +  # Adjust size of legend boxes
  guides(color = guide_legend(label.position = "top", label.hjust = 0.5)) +
...
# Create separate boxplots for merged and unmerged host contamination
p1 <- ggplot(host_clean, aes(x="", y=percent_host_in_merged))+
  ...
  xlab("") +
  scale_y_continuous(limits = c(0, 100)) +
  labs(title="Host contamination in merged reads")
...
p2 <- ggplot(host_clean, aes(x="", y=percent_host_in_unmerged))+
  ...
  xlab("") +
  scale_y_continuous(limits = c(0, 100)) +
  labs(title="Host contamination in unmerged reads")
...
# Create a bar plot showing host stats by sample
host_long <- host_clean %>%
  select(sample, percent_host_in_merged, percent_host_in_unmerged, percent_host_overall) %>%
  pivot_longer(cols = starts_with("percent_"), 
               names_to = "metric", 
               values_to = "percentage") %>%
  mutate(metric = factor(metric, 
                         levels = c("percent_host_overall", "percent_host_in_merged", "percent_host_in_unmerged"),
                         labels = c("Overall", "Merged reads", "Unmerged reads")))

ggplot(host_long, aes(x = reorder(sample, percentage), y = percentage, fill = metric)) +
  geom_col(position = "dodge") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Host contamination by sample",
       x = "Sample",
       y = "Percent host contamination",
       fill = "Read type") +
